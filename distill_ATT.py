...
def main(args):
    ...
    for it in range(0, args.Iteration+1):
        ...
        ### ATT params
        current_index = 0
        current_L_loss = 10000 # set a large enough number
        #
        ...
        for step in range(args.syn_steps):
            ...
            ### ATT selection on optimal steps, namely determine index
            with torch.no_grad():
                current_index += 1
                L_loss = torch.nn.functional.l1_loss(student_params[-1], target_params, reduction="sum")
                # L_loss = torch.nn.functional.mse_loss(student_params[-1], target_params, reduction="sum")
                if current_L_loss >= L_loss:
                    current_L_loss = L_loss
                    index = current_index
        ...
        param_loss += torch.nn.functional.mse_loss(student_params[index], target_params, reduction="sum")
        ...
        ### align test setup with FTD 
        ### ema only speed up distillation process for ema tests.
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default='datas', help='dataset path')
